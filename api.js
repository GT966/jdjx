    ...
    $.get({url: `http://api.jdsharecode.xyz/api/${db}/${num}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (data) {
          ...
          // $.msg("JDHelloWorld", "获取助力池失败！", `请手动访问https://api.jdsharecode.xyz/api/version`, {"open-url": "https://api.jdsharecode.xyz/api/version"})
...
function version() {
  return new Promise(resolve => {
    console.log('\n开始版本检测......')
    $.get({url: `https://api.jdsharecode.xyz/api/version`, timeout: 10000}, (err, resp, data) => {
      try {
        if (data) {
          ...
          $.msg("JDHelloWorld", "版本检测失败", `请手动访问 https://api.jdsharecode.xyz/api/version 测试网络`, {"open-url": "https://api.jdsharecode.xyz/api/version"})
          if ($.isNode()) {
            const notify = require('./sendNotify')
            notify.sendNotify("JDHelloWorld", `版本检测失败\n请手动访问\nhttps://api.jdsharecode.xyz/api/version\n测试网络`)
          }
        }
      } catch (e) {
        $.msg("JDHelloWorld", "版本检测失败", `请手动访问 https://api.jdsharecode.xyz/api/version 测试网络`, {"open-url": "https://api.jdsharecode.xyz/api/version"})
        if ($.isNode()) {
          const notify = require('./sendNotify')
          notify.sendNotify("JDHelloWorld", `版本检测失败\n请手动访问\nhttps://api.jdsharecode.xyz/api/version 测试网络`)
        }
      } finally {
        resolve()
      }
